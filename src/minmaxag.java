...
public class minmaxag
{
	...
	public int move(String S)
	{
		...
		//if there is no X (opponnent made first move), then find the move that the opponent made by finding the O
		if (!S.contains("X"))
		{
			System.out.println("Restart board state");
	...
	private int evalMove(String newState, int x, int y, String moveChar)
	{
		...
		else
		{
			...
			*/
				...
				for (int xCoord = 1; xCoord <= BOARD_LENGTH; xCoord++)
				{
					...
					if (prospectiveToken == MAX_CHAR || prospectiveToken == '_')
					{
						...
						if (xCoord < 5)
						{
							...
							if (!disrupted)
							{
								//add horizontal score from this space to the maxscore
						...
						if (yCoord < 4)
						{
							...
							if (!disrupted)
							{
								//add vertical score from this space to the maxscore
								maxScore += directionScore;
							}
						}
						...
						if (xCoord < 5 && yCoord < 4)
						{
							...
							if (!disrupted)
							{
								//add diagnal score from this space to the maxscore
								maxScore += directionScore;
							}
						...
						if (xCoord < 5 && yCoord > 3)
						{
							...
							if (!disrupted)
							{
								//add reverse diagnal score from this space to the maxscore
								maxScore += directionScore;
							}
						}
					}
			...
			//return the max score if there is no winner or tie
			return maxScore;
		}
	}
